...
const API = "http://192.168.31.6:8000/api"

const AVAILABLE_ROLES = [
  'Data Scientist', 'Data Analyst', 'Data Engineer',
  'ML Engineer', 'Business Analyst', 'AI Engineer',
  'BI Developer', 'Python Developer'
]
  ...
  return (
    <div className="auth-wrapper">
      ...
      <form className="auth-card" onSubmit={handleSubmit}>
        ...
        <div style={{ display: 'flex', background: 'rgba(255,255,255,0.05)', borderRadius: 10, padding: 4, marginBottom: 20, gap: 4 }}>
          <button
            ...
            style={{
              ...
              background: !isRegister ? 'var(--accent-indigo)' : 'transparent',
        ...
        <h2>{isRegister ? 'Create New Account' : 'Welcome Back'}</h2>

        <div className="form-group">
          <label>Username</label>
          <input type="text" placeholder={isRegister ? "Choose a username (min 3 chars)" : "Enter your username"} value={username}
            onChange={(e) => setUsername(e.target.value)} />
        </div>
        <div className="form-group">
          <label>Password</label>
          <input type="password" placeholder={isRegister ? "Create a password (min 4 chars)" : "Enter your password"} value={password}
        ...
        <button className="btn-primary" type="submit" disabled={loading}>
          ...
          {isRegister ? '🚀 Create Account & Continue' : '🔑 Login'}
        </button>

        <p style={{ textAlign: 'center', fontSize: 12, color: 'var(--text-muted)', marginTop: 14 }}>
          {isRegister ? 'Already have an account?' : "Don't have an account?"}
          {' '}
          <span style={{ color: 'var(--accent-indigo)', cursor: 'pointer', fontWeight: 600 }} onClick={switchMode}>
    ...
    <>
      ...
      <div className="section-card">
        ...
        <div className="form-group">
          <input 
            type="text" 
            placeholder="Type a role (e.g. Frontend Developer) and press Enter" 
            value={roleInput}
        ...
        <div className="role-chips">
          ...
          {AVAILABLE_ROLES.filter(r => !roles.includes(r)).map(role => (
            <div key={role} className="role-chip" onClick={() => toggleRole(role)}>
              + {role}
            </div>
          ))}
        </div>
      ...
      <div className="section-card">
        ...
        <div className={`upload-box ${dsUploaded ? 'uploaded' : ''}`}
          ...
          <div className="upload-label">Data Science Resume (PDF)</div>
          {!dsUploaded && <div className="upload-hint">Click to upload or drag & drop</div>}
          {dsUploaded && <div className="upload-status">Uploaded</div>}
          <input ref={dsRef} type="file" accept=".pdf" style={{ display: 'none' }}
            onChange={(e) => e.target.files[0] && uploadResume('ds', e.target.files[0])} />
        </div>
        <div className={`upload-box ${daUploaded ? 'uploaded' : ''}`}
          onClick={() => daRef.current?.click()}>
          <div className="upload-icon">{daUploaded ? '✅' : '📤'}</div>
          <div className="upload-label">Data Analyst Resume (PDF)</div>
          {!daUploaded && <div className="upload-hint">Click to upload or drag & drop</div>}
          {daUploaded && <div className="upload-status">Uploaded</div>}
          <input ref={daRef} type="file" accept=".pdf" style={{ display: 'none' }}
            onChange={(e) => e.target.files[0] && uploadResume('da', e.target.files[0])} />
        </div>
      </div>
